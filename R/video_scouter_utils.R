...
guess_dig_player_options <- function(game_state, dvw, system, weighted = TRUE) {
    ...
    dig_responsibility <- player_responsibility_fn(system = system, skill = "Dig", setter_position = setter_rot, zone = defending_zone, libs = libs,
                                                   ...
                                                   n_blockers = if (tail(dvw$plays$skill, 1) %eq% "Attack") tail(dvw$plays$num_players_numeric, 1) else NA_integer_)
...
do_video_inner <- function(what, ..., video_state, rally_state, app_data, session, id = "main_video") {
    ...
    invisible({
        if (what == "pause") {
            ...
            if (rally_state() == .C_click_serve_start) set_rally_state(.C_click_to_start_msg)
            TRUE ## paused state
        } else if (what == "play") {
            dojs(paste0(
            ...
            if (rally_state() == .C_click_to_start_msg) set_rally_state(.C_click_serve_start)
