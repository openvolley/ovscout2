ov_shiny_video_sync_ui <- function(app_data) {
    ...
    fluidPage(theme=if (running_locally) "spacelab.css" else "https://cdnjs.cloudflare.com/ajax/libs/bootswatch/3.3.7/spacelab/bootstrap.min.css",
              ...
              fluidRow(column(7, 
                              introBox(tags$video(id = "main_video", style = "border: 1px solid black; width: 90%;", src = file.path(video_server_base_url, basename(video_src)), controls = "controls", autoplay = "false"), data.step = 4, data.intro = "Video of the game to scout. Controls are shown inside the video frame."),
                              fluidRow(column(8, 
                                              ...
                                              actionButton("ovscout_helper", label = tags$span(icon("keyboard"), "Keyboard shortcuts"), icon = icon("question"), style="color: #fff; background-color: #B21212; border-color: #B21212"),
