...
#'   
#'   toreplace <- x$plays[x$plays$point_id %in% new_x$new_rotation$point_id, colnames(new_x$new_rotation)]
#'   new_xx <- dplyr::left_join(dplyr::select(toreplace, point_id), new_x$new_rotation)
#'   
#'   x$plays[x$plays$point_id %in% new_x$new_rotation$point_id, colnames(new_x$new_rotation)] <- new_xx
#'   
#' }
#' @export
rotations <- function(x, team, start_point_id, set_number, new_rotation = NULL){
    ...
    if(missing(set_number)) set_number_select = 1 else set_number_select = set_number
    ...
    if(teamSelect == home_team(x)) player_table <- dplyr::select(x$meta$players_h, 'number', 'player_id', 'special_role', 'role')
    if(teamSelect == visiting_team(x)) player_table <- dplyr::select(x$meta$players_v, 'number', 'player_id', 'special_role', 'role')
    ...
    if(teamSelect == home_team(x)) x_tmp = dplyr::distinct(dplyr::filter(dplyr::select(x$plays, 'point_id','skill', tidyselect::starts_with("home_player_id")), .data$point_id %in% point_ids, .data$skill %in% c("Serve", "Timeout")))
    
    if(teamSelect == visiting_team(x)) x_tmp = dplyr::distinct(dplyr::filter(dplyr::select(x$plays, 'point_id', 'skill', tidyselect::starts_with("visiting_player_id")), .data$point_id %in% point_ids, .data$skill %in% c("Serve", "Timeout")))
    ...
    if(!is.null(new_rotation)){
        ...
        if(teamSelect == visiting_team(x)){
            ...
            x_tmp_long$new_p <- as.numeric(x_tmp_long$new_p)
            x_tmp_long_new <- dplyr::rename(dplyr::select(dplyr::left_join(x_tmp_long, dplyr::select('player_table','number', 'player_id'), by = c("new_p" = "number")),
                                                          'point_id', 'position', 'new_p', 'player_id'), "home_p" = "new_p", "visiting_player_id" = "player_id")
