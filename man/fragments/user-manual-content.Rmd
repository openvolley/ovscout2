...
The app is writen in R, but you do not necessarily need to be an R user to be able to use it. We provide script files to launch the app from the command line (although you will need to install R itself on some platforms, see below). If you are not an R user, see the "Standalone" section below for installation and startup.
...
3. [pandoc](https://github.com/jgm/pandoc/) is required for generating match reports from scouted files. Install following https://github.com/jgm/pandoc/blob/master/INSTALL.md. If not present, the report generation menu item won't be shown.
...
There are several other options that can be provided to the `ov_scouter()` function, including:

- `auto_save_dir` is a directory into which the scouted file will be saved (in .dvw format) after each rally. You can use this to e.g. provide live updates to the coaching bench
...
- `scouting_options`, `default_scouting_table`, `compound_table` - advanced options controlling details of the scouting process

Options controlling the app behaviour and scouting preferences can be changed via the "Preferences" button in the app.

# Preferences

A number of preferences can be set, either via the "Preferences" button in the app or (if you are starting the app from within R) via `ov_scouting_options` and other arguments to the `ov_scouter` function. Preferences include:

- "End convention" determines whether you are treating the end coordinate of an attack or serve as the **actual** end location (i.e. where the ball contacted the floor or out of bounds area), or the **intended** end location. The actual might differ from the intended if there is a block touch or the ball hit the net. If 'actual', and a block touch is recorded, then the end location of the attack will not be used for the dig location (the dig location will be missing).

- "Record sets in transition". If `FALSE`, then sets in transition are not scouted. After entering an attack end location (and play continues) the next click will be for the counterattack start location, with no set scouted in between.
