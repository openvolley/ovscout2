...
1. `ffmpeg` is used to extract frames from videos, and other video-related functions. If `ffmpeg` is not available, an internal fallback method is used for some functionality, but may be considerably slower. Install `ffmpeg` via your package manager, or (from within R, on Linux and Windows only) using `ovideo::ov_install_ffmpeg()`
...
After an attack or freeball over, click the end point (point where the ball lands, or the point on the court directly below the ball when it is played).
