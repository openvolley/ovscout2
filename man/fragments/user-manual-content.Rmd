...
1. Install R, following <https://cran.r-project.org/bin/windows/base/>. Ensure that the "Save version number to registry" box is ticked during installation.
...
Keyboard shortcuts can be viewed by clicking the `r tags$button(tags$span(icon("keyboard"), "Keyboard", tags$br(), "shortcuts"), class = "leftbut")` button.
...
If the ball deflects off the block or the net, and you want the ball path to show this exactly, you can do this by entering the attack as normal, and then going back afterwards (at the end of the rally, perhaps) and editing the coordinates. Click the `r icon("location-dot")` icon underneath the court rotation diagram and you will be prompted to enter new coordinates for the attack, including a middle coordinate that can be used to show the block deflection.
...
You can switch between click-scouting and type-scouting with the `r tags$button(tags$span(icon("repeat"), "Change to click", tags$br(), "scout mode"), class = "btn btn-default leftbut")` button.
