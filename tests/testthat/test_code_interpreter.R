...
test_that("code interpretation works correctly", {
    ...
    check_code(ov_code_interpret("*5X5!"), "*05AT!X5F4~~H")
    check_code(ov_code_interpret("5X5!"), "*05AT!X5F4~~H")
    check_code(ov_code_interpret("5X5"), "*05AT+X5F4~~H") ## default evaluation
    check_code(ov_code_interpret("5X55"), "*05AT+X5F45~H") 
    check_code(ov_code_interpret("*5X5+5H2"), "*05AT+X5F45~H2")
    check_code(ov_code_interpret("*5X55H2"), "*05AT+X5F45~H2")
    ...
    check_code(ov_code_interpret("2X5#.15D"), c("*02AT#X5F4", "a15DT=")) ## but we can specify e.g. dig error.
    check_code(ov_code_interpret("2SQ1.5=4"), c("*02SQ#~~~14", "a05RQ=~~~14"))
    check_code(ov_code_interpret("2X5#5S"), "*02AT#X5F45~H~S") ## Include special syntax
})
